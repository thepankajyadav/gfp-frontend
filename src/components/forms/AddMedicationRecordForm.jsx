...
const AddMedicationRecordForm = ({ isOpen, onClose, onSubmit, animalTag }) => {
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        onSubmit(formData);
        setFormData({ animalTag: animalTag || '', medicine: '', date: '', dosage: '', administeredBy: '' });
        onClose();
    };

    return (
        <Modal isOpen={isOpen} onClose={onClose} title="Add Medication Record">
            <form onSubmit={handleSubmit}>
                ...
                <div style={{ marginBottom: '1rem' }}>
                    ...
                    <input type="text" name="medicine" value={formData.medicine} onChange={handleChange} required
                ...
                <div style={{ marginBottom: '1rem' }}>
                    <label style={{ display: 'block', marginBottom: '0.5rem', fontSize: '0.9rem', fontWeight: '500' }}>Dosage</label>
                    <input type="text" name="dosage" value={formData.dosage} onChange={handleChange} required placeholder="e.g., 5ml"
                        style={{ width: '100%', padding: '0.75rem', border: '1px solid var(--color-border)', borderRadius: 'var(--radius-sm)', outline: 'none' }} />
                </div>
...
export default AddMedicationRecordForm;
