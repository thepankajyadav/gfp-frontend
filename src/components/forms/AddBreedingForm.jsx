...
const AddBreedingForm = ({ isOpen, onClose, onSubmit }) => {
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        onSubmit(formData);
        setFormData({ maleTag: '', femaleTag: '', date: '', status: 'Pending' });
        onClose();
    };
        ...
        <Modal isOpen={isOpen} onClose={onClose} title="Add Breeding Record">
            <form onSubmit={handleSubmit}>
                <div style={{ marginBottom: '1rem' }}>
                    <label style={{ display: 'block', marginBottom: '0.5rem', fontSize: '0.9rem', fontWeight: '500' }}>Male Tag</label>
                    <input type="text" name="maleTag" value={formData.maleTag} onChange={handleChange} required placeholder="e.g., GT-001"
                ...
                <div style={{ display: 'flex', gap: '1rem', justifyContent: 'flex-end' }}>
                    ...
                    <Button type="submit" variant="primary">Add Breeding Record</Button>
                </div>
            </form>
        </Modal>
    );
